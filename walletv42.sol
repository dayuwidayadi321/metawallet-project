...
/**
 * @title AdvancedSmartWallet - EIP-4337 Smart Wallet (v4.2)
 * @author BY DFXC INDONESIA WEB3 PROJECT
 * @notice Smart wallet with anti-front-running protection, owner tracking, and enhanced security
 * @dev Major improvements:
 * - Anti-front-running protection for recovery process
 * - Complete owner list tracking with events
 * - Better recovery process validation
 */
contract AdvancedSmartWallet is IERC1271, Initializable, UUPSUpgradeable, ReentrancyGuard {
    using ECDSA for bytes32;

    string public constant VERSION = "4.2";
    string public name;
    ...
    address[] public ownerList; // Track all owners in an array
    ...
    struct RecoveryConfig {
        ...
        uint256 delay;
        uint256 initiatedAt;
        address pendingNewOwner; // For anti-front-running protection
    }
    ...
    event OwnershipUpdated(address[] newOwners); // New event for tracking owner changes
    ...
    // Perbaikan: Pastikan ownerList tidak kosong dan hapus dengan aman
        uint256 lastIndex = ownerList.length - 1;
        for (uint256 i = 0; i <= lastIndex; i++) {
            if (ownerList[i] == ownerToRemove) {
                if (i != lastIndex) {
                    ownerList[i] = ownerList[lastIndex]; // Geser elemen terakhir ke posisi yang dihapus
                }
...
/**
 * @title AdvancedSmartWalletFactory v4.2
