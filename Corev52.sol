 ...
 * @dev [IMPROVED] Critical security fixes applied to recovery mechanism:
 *      - Fixed replay attack vulnerability in initiateRecovery()
 *      - Added global nonce instead of per-caller nonce
 *      - Implemented atomic owner replacement
 *      - Added duplicate signature checks
 *      - Added deadline validation
 *      - Improved input validation for new owners
 */
...
abstract contract SecureSmartWalletCore is Initializable, EIP712 {
    ...
    function initiateRecovery(address[] calldata newOwners, bytes[] calldata signatures, uint256 deadline) external {
        ...
        // Use global recovery nonce instead of per-caller
        bytes32 recoveryHash = _hashTypedDataV4(keccak256(abi.encode(
            RECOVERY_TYPEHASH,
            keccak256(abi.encodePacked(newOwners)),
            recoveryNonces[address(this)]++, // Use contract-level nonce
            deadline
        )));
        ...
        EnumerableSet.AddressSet memory newOwnerSet = EnumerableSet.AddressSet();
        
        // First add new owners to temporary set
        for (uint i = 0; i < newOwners.length; i++) {
            require(newOwners[i] != address(0), "Invalid owner");
            require(newOwnerSet.add(newOwners[i]), "Duplicate owner");
        }
        
        // Then clear existing owners
        while (owners.length() > 0) {
            owners.remove(owners.at(0));
        }
        
        // Finally add new owners
        for (uint i = 0; i < newOwners.length; i++) {
            owners.add(newOwners[i]);
        }
