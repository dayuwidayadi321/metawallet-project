...
abstract contract SecureSmartWalletSignatures is SecureSmartWalletBase {
    ...
    function scheduleOperation(
        ...
        require(
            _isOwner(signer) || _isGuardian(signer), 
            "Invalid signer"
        );
    ...
    function cancelOperation(bytes32 opHash) external {
        ...
        require(operation.opHash == opHash, "Operation not found");
        require(!operation.executed, "Operation already executed");
        require(msg.sender == operation.initiator || _isOwner(msg.sender), "Not authorized");
    ...
    function _isValidSigner(
        ...
        if (_isOwner(signer)) return true;
        if (!_isGuardian(signer)) return false;
