    ...
    IERC1271Upgradeable
{
    ...
    string public constant VERSION = "4.49.0"; // Updated version
    ...
    constructor(IEntryPoint _entryPoint) 
        SecureSmartWalletBase(_entryPoint, address(this)) 
    {
    ...
    function isValidSignature(bytes32 hash, bytes calldata signature) 
        ...
        returns (bytes4) 
    {
        // Access storage variables through proper inheritance
        if (SecureSmartWalletEmergency.isLocked) return bytes4(0xffffffff);
        ...
        if (SecureSmartWalletEmergency.guardianRequired) {
    ...
    function validateSignaturesBatch(
        ...
        for (uint i = 0; i < hashes.length; i++) {
            if (isGuardianCheck[i] && SecureSmartWalletEmergency.guardianRequired) {
                results[i] = SecureSmartWalletSignatures._validateGuardianSignature(hashes[i], signatures[i]);
            } else {
                (bool isValid,) = SecureSmartWalletSignatures._validateOwnerSignature(hashes[i], signatures[i]);
                results[i] = isValid;
            }
        }
        return results;
    ...
    function _authorizeUpgrade(address newImplementation) 
        ...
        onlyOwner
    {
        require(newImplementation != address(0) && newImplementation != address(this), "Invalid implementation");
...
/**
 * @title SecureSmartWalletFactory (Optimized)
 * @notice Factory with CREATE2 for deterministic deployments
 */
