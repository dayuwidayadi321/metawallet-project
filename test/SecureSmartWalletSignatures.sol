...
abstract contract SecureSmartWalletSignatures is SecureSmartWalletBase {
    ...
    function _validateSignature(bytes32 hash, bytes memory signature) 
        ...
        returns (bool) 
    {
        // Early exit untuk signature kosong
        if (signature.length == 0) return false;
        
        bytes32 ethSignedMessageHash = MessageHashUtilsUpgradeable.toEthSignedMessageHash(hash);
        address signer = ethSignedMessageHash.recover(signature);
        
        // Check owner terlebih dahulu karena lebih sering terjadi
        if (isOwner[signer]) return true;
        
        // Skip check contract jika tidak diperlukan
