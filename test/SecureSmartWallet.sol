    ...
    IERC1271Upgradeable
{
        ...
        returns (bytes4) 
    {
        ...
        if (isOwner) {
            // Can't emit events in view functions - removed for compatibility
    ...
    function validateSignaturesBatch(
        ...
        require(hashes.length == signatures.length, "Hashes/signatures length mismatch");
        require(signatures.length == isGuardianCheck.length, "Signatures/checks length mismatch");
