    ...
    IERC1271Upgradeable
{
    ...
    // ===================== State Variables =====================
    // Removed _initialized (using Initializable's built-in)
        ...
        returns (bytes4) 
    {
        ...
        bool currentLockStatus = _isLocked;
        bool currentGuardianRequired = guardianRequired;
        
        if (currentLockStatus) return bytes4(0xffffffff);
    ...
    function validateSignaturesBatch(
        ...
        for (uint i = 0; i < hashes.length; i++) {
            if (isGuardianCheck[i] && currentGuardianRequired) {
