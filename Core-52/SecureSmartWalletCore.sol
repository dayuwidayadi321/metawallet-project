...
abstract contract SecureSmartWalletCore is Initializable, EIP712 {
    ...
    function __SecureSmartWalletCore_init_with_guardians(
        address[] calldata initialOwners,  // Renamed
        address[] calldata initialGuardians,  // Renamed
        ...
        for (uint i = 0; i < initialOwners.length; i++) {
            _owners.add(initialOwners[i]);  // Now correctly references state variable
        }
    
        // Set guardians
        for (uint i = 0; i < initialGuardians.length; i++) {
            _guardianConfig.guardians.add(initialGuardians[i]);
        }
