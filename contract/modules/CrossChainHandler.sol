...
abstract contract CrossChainHandler is ILayerZeroReceiver {
    ...
    uint256 private _callbackStatus;
    ...
    modifier onlyActiveChain(uint16 chainId) {
        require(isChainActive[chainId], "Chain inactive");
        _;
    }

    modifier noReentrancy() {
        require(_callbackStatus == 0, "Reentrancy guard");
        _callbackStatus = 1;
        _;
        _callbackStatus = 0;
    }
    
    modifier onlyActiveChain(uint16 chainId) override {
        require(isChainActive[chainId], "Chain inactive");
        _;
    }    
    ...
    function setMinGasLimit(
        ...
        require(_minGas <= MAX_GAS_LIMIT, "Exceeds max gas");
    ...
    function sendCrossChain(
        ...
        bytes calldata _adapterParams
    ) external payable virtual onlyOwner onlyActiveChain(_dstChainId) {
    ...
    function lzReceive(
        ...
        uint64,
        bytes calldata _payload
    ) external override noReentrancy {
