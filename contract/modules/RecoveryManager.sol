    ...
    PausableUpgradeable 
{
    ...
    EnumerableSetUpgradeable.AddressSet internal _owners;
    ...
    function initiateRecovery(
        ...
        require(newOwners.length >= MIN_NEW_OWNERS && newOwners.length <= MAX_NEW_OWNERS, 
            "Recovery: Invalid owner count");
        ...
        // Hitung dan validasi hash owners
        ...
        // Generate dan validasi recovery hash
        bytes32 recoveryHash = _hashTypedDataV4(keccak256(abi.encode(
            RECOVERY_TYPEHASH,
            keccak256(abi.encodePacked(newOwners)), // ownersHash
            recoveryNonce,
            deadline,
            address(this),
            block.chainid
        ...
        for (uint256 i = 0; i < signatures.length; i++) {
            ...
            if (_guardianConfig.guardians.contains(signer)) {
                ...
                if (!alreadySeen) {
                    ...
                    // Early exit jika threshold tercapai
    ...
    function _replaceOwners(address[] calldata newOwners) internal virtual {
        ...
        // Hapus existing owners dalam urutan terbalik
        for (uint i = ownerCount; i > 0; i--) {
            _owners.remove(_owners.at(i - 1));
        }
        
        // Tambah new owners dengan validasi
     ...
     */
    function incrementRecoveryNonce(address guardian) external virtual onlyOwner {
        require(_guardianConfig.guardians.contains(guardian), "Recovery: Not a guardian");
        uint256 newNonce = ++guardianRecoveryNonces[guardian];
        emit GuardianRecoveryNonceIncremented(guardian, newNonce);
    }

    // Storage gap untuk future upgrades
