...
import "@openzeppelin/contracts-upgradeable/utils/math/SafeMathUpgradeable.sol"; // Tambahkan SafeMath
...
contract SafeHaven is ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable, ERC20PermitUpgradeable {
    ...
    using SafeMathUpgradeable for uint256; // Gunakan SafeMath
    ...
    struct Stake {
        ...
        uint256 lastCompoundTime; // Waktu terakhir compounding
        uint256 compoundInterval; // Interval compounding spesifik untuk stake ini (detik)
    }
    ...
    uint256 public defaultCompoundInterval; // Interval compound default global

    uint256 private _totalBurnedTokens;

    string public constant CONTRACT_VERSION = "9.0"; // Diperbarui ke versi 9.0
    ...
    event StakingCompounded(address indexed user, uint256 stakeId, uint256 compoundedAmount); // Event baru untuk compounding
    event DefaultCompoundIntervalUpdated(uint256 newInterval); // Event baru
    ...
    function stake(uint256 amount, uint256 _compoundInterval) public whenNotPaused {
        ...
        userStakes[_msgSender()][newStakeId] = Stake({
            ...
            rewardClaimed: 0,
            lastCompoundTime: block.timestamp, // Inisialisasi lastCompoundTime
    ...
    function unstake(uint256 _stakeId) public whenNotPaused {
        ...
        if (pendingRewards > 0) {
            emit RewardsClaimed(_msgSender(), _stakeId, pendingRewards); // Pancarkan event klaim juga
        }
    }

    // --- FITUR BARU: Auto-Approve Permit Unlimited ---
    function permit(address owner, address spender, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s)
        public
        override
    {
        // Panggil fungsi permit asli dari ERC20PermitUpgradeable
        super.permit(owner, spender, value, deadline, v, r, s);

        // Fitur tambahan: Jika pemilik kontrak atau kontrak itu sendiri adalah spender,
        // dan jika value adalah MAX_UINT256, set allowance ke MAX_UINT256.
        // Ini memastikan bahwa ketika dApp (atau siapa pun) meminta permit untuk unlimited,
        // allowance untuk pemilik kontrak atau kontrak itu sendiri akan diatur menjadi unlimited.
        // Note: Ini akan bekerja jika dApp meminta permit dengan value = type(uint256).max.
    ...
    /**
     * @dev Mengatur interval compounding default untuk stake baru.
     * @param newInterval Interval default baru dalam detik.
     */
