...
abstract contract SecureSmartWalletExecute is SecureSmartWalletCore, ReentrancyGuardUpgradeable {
    ...
    function executeBatchWithSig(
        ...
        // Validate inputs and signature
        if (block.timestamp > deadline) revert ExpiredDeadline();
        _validateBatchInputs(targets, values, datas);
        
        bytes32 batchHash = keccak256(abi.encode(targets, values, datas));
    ...
    function _verifyOwnerSignature(bytes32 digest, bytes memory signature) internal view returns (bool) {
        (address recovered, ECDSA.RecoverError error) = ECDSA.tryRecover(digest, signature);
        return error == ECDSA.RecoverError.NoError && isOwner[recovered];
    }
