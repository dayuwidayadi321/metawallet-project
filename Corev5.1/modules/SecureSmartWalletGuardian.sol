...
abstract contract SecureSmartWalletGuardian is Initializable, EIP712Upgradeable, SecureSmartWalletCore {
    ...
    address[] public activeGuardians; // Track guardians separately for iteration
    ...
    modifier onlyActiveGuardian() {
        require(_isActiveGuardian(msg.sender), "Unauthorized: Not active guardian");
        _;
    }
    ...
    // Batch update guardians with EIP-712 signature
    ...
    // Create time-bound mandate for a guardian
    ...
    function isFunctionAllowedForGuardian(
        address guardian,
        bytes4 selector
    ) external view returns (bool) {
        GuardianMandate memory mandate = guardianMandates[guardian];
        return _isFunctionAllowed(selector, mandate.allowedFunctions);
    }
