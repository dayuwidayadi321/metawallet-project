 ...
 * @notice Version 5.5.2 with Critical Security Enhancements  
 * @dev Audit Phase #7 - Critical Fixes:  
 *      - [CRITICAL] Fixed remaining reentrancy vulnerability in plugin execution  
 *      - [SECURITY] Added permanent lock during plugin delegatecall  
 *      - [SECURITY] Implemented dangerous selector blacklist  
 *      - [GAS] Optimized plugin execution gas costs  
 *  
 * Key Improvements:  
 * [FIXED] Complete reentrancy protection in `_executePlugin`  
 * [ADDED] State lock persistence throughout plugin execution  
 * [UPGRADED] Stronger gas limit enforcement  
 *  
 * Security Model:  
 * - EIP-4337 + ERC-7201 (Reentrancy Protection Standard)  
 * - Double-layer reentrancy guards (modifier + manual lock)  
 * - Time-locked upgrades with automatic rollback  
 */
...
abstract contract CoreV55 is Initializable, UUPSUpgradeable, EIP712Upgradeable, ReentrancyGuardUpgradeable, ERC1967UpgradeUpgradeable {
    ...
    constructor(
        ...
        // Validasi parameter dengan custom error
        if (address(_entryPoint) == address(0)) revert InvalidAddress("entryPoint");
        if (_gasOracle == address(0)) revert InvalidAddress("gasOracle");
        if (initialOwners.length == 0) revert InvalidParameter("initialOwners");
        if (initialGuardian == address(0)) revert InvalidAddress("initialGuardian");
        if (_lzEndpoint == address(0)) revert InvalidAddress("lzEndpoint");
        if (_supportedChainIds.length != _trustedRemotes.length) revert MismatchedArrayLengths();
        
        // Inisialisasi immutable
        ...
        emit WalletConstructed(address(_entryPoint), _gasOracle);
    }
    ...
    /**
     * @dev Executes a plugin function with enhanced security checks
     * @param plugin Address of the plugin contract
     * @param data Calldata for the plugin function
     * @notice Adds multiple layers of reentrancy protection and strict validation
     */
    function _executePlugin(
        ...
        // Check 3: Block dangerous selectors
        require(
            selector != 0x9e5faafc && selector != 0x71e4150e,
            "Dangerous selector blocked"
        );
    
        // Check 4: Pre-flight staticcall check
        (bool preCheckSuccess, ) = plugin.staticcall(data);
        require(preCheckSuccess, "Plugin pre-check failed");
    
        // Check 5: Gas limit protection
        uint256 gasBefore = gasleft();
        require(gasBefore > 100_000, "Insufficient gas");
    
        // ========== EXECUTION ========== //
        // Lock the contract state before delegatecall
        bool previousLockState = env.isLocked;
        env.isLocked = true;
    
        // Execute plugin with delegatecall
        (bool success, bytes memory result) = plugin.delegatecall(data);
    
        // ========== POST-EXECUTION CHECKS ========== //
        // Restore lock state regardless of execution result
        env.isLocked = previousLockState;
    ...
    error PluginPreCheckFailed();
    error InsufficientGas();
    error ExcessiveGasUsage();
    error PluginExecutionFailed(bytes reason);
