 ...
 * @notice Version 5.7.1 with Critical Security Upgrades  
 * @dev Audit Phase #7 - Major Fixes:  
 *      - [CRITICAL] Fixed upgrade reentrancy during health checks  
 *      - [SECURITY] Added mutex lock for atomic upgrades  
 *      - [GAS] Optimized post-upgrade validation with static calls  
 *  
 * Key Improvements:  
 * [FIXED] Reentrancy in `executeUpgrade()` and `rollbackUpgrade()`  
 * [ADDED] Block-based rollback window (5 blocks)  
 * [UPGRADED] Hardened storage consistency checks  
 *  
 * Security Model:  
...
abstract contract CoreV57 is Initializable, UUPSUpgradeable, EIP712Upgradeable, ReentrancyGuardUpgradeable, ERC1967UpgradeUpgradeable {
    ...
    /* ========== PLUGIN SYSTEM CoreV57 (ULTIMATE MODULAR) ========== */
    ...
    function removeGuardian(address guardian) external virtual onlyOwner {
        require(_guardianConfig.guardians.contains(guardian), 
            string(abi.encodePacked("CoreV57: Address 0x", _toHexString(guardian), " is not a guardian")));
    ...
    function _validateImplementation(address newImpl) internal view {
        ...
        require(bytes(versionStr).length == 3 && keccak256(bytes(versionStr)) == keccak256(bytes("5.7")), "Incompatible version");
    ...
    function checkStorageConsistency() external view returns (bool) {
        return address(env.entryPoint) != address(0) &&
               ...
               keccak256(bytes(this.VERSION())) == keccak256(bytes("5.7"));
    }
    
        /* ========== INTERNAL SAFE CHECKS ========== */
    function _safePostUpgradeCheck() internal view returns (bool) {
        // Check 1: Versi harus tetap "5.7"
        try this.VERSION() returns (string memory v) {
            if (keccak256(bytes(v)) != keccak256(bytes("5.7"))) return false;
        } catch {
            return false;
        }
    ...
    function checkUpgradeHealth() public returns (bool) {
        ...
        try this.VERSION{gas: 50_000}() returns (string memory v) {
            isHealthy = keccak256(bytes(v)) == keccak256(bytes("5.7"));
        } catch {
            return false;
        }
        ...
        whenNotLocked 
    {
        ...
        require(success && data.length > 0, "Version check failed");
        string memory versionStr = abi.decode(data, (string));
        require(keccak256(bytes(versionStr)) == keccak256(bytes("5.7")), "Version mismatch");
