...
/**
 * @title CoreV58 - Inheritable EIP-4337 Smart Wallet with Multi-Chain Support  
 * @author DFXC IndonesiaSecurity Web3 Team - Developed by Dayu Widayadi  
 * @notice Version 5.8 with Critical Security Upgrades  
 * @dev Audit Phase #8 - Major Fixes:  
 *      - [CRITICAL] Fixed plugin uninstall race condition  
 *      - [SECURITY] Added plugin uninstall state tracking  
 *      - [GAS] Optimized plugin execution checks  
 *  
 * Key Improvements:  
 * [FIXED] Plugin execution during uninstall window  
 * [ADDED] isUninstalling flag for plugins  
 * [UPGRADED] Enhanced plugin security checks  
 *  
 * Security Model:  
 * - EIP-4337 + ERC-7201 (Reentrancy Protection Standard)  
 * - Guardian multi-sig with chain-bound nonces  
 * - Time-locked atomic upgrades  
...
import "./modul/ReentrancyGuard.sol"; // Fixed warning
...
abstract contract CoreV58 is Initializable, UUPSUpgradeable, EIP712Upgradeable, ReentrancyGuardUpgradeable, ERC1967UpgradeUpgradeable {
    ...
    /* ========== mutex untuk upgrade ======== */
    bool private _upgradeMutex;
    uint256 private _upgradeBlockNumber;    
    ...
    /* ========== PLUGIN SYSTEM CoreV58 (ULTIMATE MODULAR) ========== */
    ...
    function uninstallPlugin(address plugin) external virtual onlyOwner {
        ...
        _pluginSystem.installedPlugins[plugin].isUninstalling = true; // Mark as uninstalling
     ...
     */
    function _executePlugin(address plugin, bytes calldata data) internal nonReentrant {
        Plugin storage p = _pluginSystem.installedPlugins[plugin];
        
        require(p.installTimestamp > 0, "Plugin not installed");
        require(!p.isUninstalling, "Plugin is being uninstalled"); // Pengecekan baru
        require(
            _pluginSystem.pluginUninstallTimestamps[plugin] == 0,
            "Plugin pending uninstall"
        );
    ...
    function removeGuardian(address guardian) external virtual onlyOwner {
        require(_guardianConfig.guardians.contains(guardian), 
            string(abi.encodePacked("CoreV58: Address 0x", _toHexString(guardian), " is not a guardian")));
    ...
    function _validateImplementation(address newImpl) internal view {
        ...
        require(bytes(versionStr).length == 3 && keccak256(bytes(versionStr)) == keccak256(bytes("5.8")), "Incompatible version");
    ...
    /* ========== UPGRADE MECHANISM (FIXED) ========== */
    function executeUpgrade() external virtual onlyOwner whenNotLocked nonReentrant {
        // Lock mutex dan catat block number
        require(!_upgradeMutex, "Upgrade already in progress");
        ...
        // Simpan implementasi sebelumnya
        previousImplementation = _getImplementation();

        // Validasi implementasi baru SEBELUM upgrade
        _validateImplementation(pendingImplementation);

        // Eksekusi upgrade
        _upgradeTo(pendingImplementation);

        // Health check TANPA interaksi eksternal
        require(_safePostUpgradeCheck(), "Post-upgrade check failed");

        // Reset state
        pendingImplementation = address(0);
        _upgradeMutex = false;
        _upgradeBlockNumber = 0;
    ...
    function checkStorageConsistency() external view returns (bool) {
        return address(env.entryPoint) != address(0) &&
               ...
               keccak256(bytes(this.VERSION())) == keccak256(bytes("5.8"));
    }
    
        /* ========== INTERNAL SAFE CHECKS ========== */
    function _safePostUpgradeCheck() internal view returns (bool) {
        // Check 1: Versi harus tetap "5.8"
        try this.VERSION() returns (string memory v) {
            if (keccak256(bytes(v)) != keccak256(bytes("5.8"))) return false;
        } catch {
            return false;
        }

        // Check 2: Storage consistency (static call)
        (bool success, bytes memory data) = address(this).staticcall(
            abi.encodeWithSignature("checkStorageConsistency()")
        );
        if (!success || !abi.decode(data, (bool))) return false;

        // Check 3: Owner tidak kosong
    ...
    function checkUpgradeHealth() public returns (bool) {
        ...
        try this.VERSION{gas: 50_000}() returns (string memory v) {
            isHealthy = keccak256(bytes(v)) == keccak256(bytes("5.8"));
        } catch {
            return false;
        }
    ...
    function rollbackUpgrade() external onlyOwner nonReentrant {
        require(_upgradeMutex, "No upgrade in progress");
        require(previousImplementation != address(0), "No previous version");
        require(block.number <= _upgradeBlockNumber + 5, "Rollback window expired"); // 5 block buffer

        // Rollback harus gagal jika state tidak konsisten
        require(!_safePostUpgradeCheck(), "Current impl healthy");
        ...
        whenNotLocked 
    {
        ...
        require(success && data.length > 0, "Version check failed");
        string memory versionStr = abi.decode(data, (string));
        require(keccak256(bytes(versionStr)) == keccak256(bytes("5.8")), "Version mismatch");
