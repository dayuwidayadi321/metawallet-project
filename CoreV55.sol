...
abstract contract CoreV55 is Initializable, UUPSUpgradeable, EIP712, ReentrancyGuard {
    ...
    function _validatePlugin(address plugin, bytes4[] calldata selectors) internal view virtual {
        ...
        // Tambahkan validasi ukuran plugin
        uint256 size;
        assembly {
            size := extcodesize(plugin)
        }
        require(size > 0 && size < 24576, "Invalid plugin size");
    
    bytes4[5] memory protectedSelectors = [
        ...
        // Validasi bahwa selector tidak kosong
        require(selectors.length > 0, "No selectors provided");
    }
    ...
    function _validateImplementation(address newImpl) internal view {
        ...
        require(success && keccak256(data) == keccak256(bytes("5.5")), "Incompatible version");
    }
