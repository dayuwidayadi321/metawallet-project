...
abstract contract CoreV55 is Initializable, UUPSUpgradeable, EIP712Upgradeable, ReentrancyGuardUpgradeable {
    ...
    function sendCrossChain(
        ...
        if (_gasPaymentToken == address(0)) {
            ...
            // Approve dan kirim
            IERC20Upgradeable(_gasPaymentToken).approve(address(lzEndpoint), gasFee);
    ...
    function _validateImplementation(address newImpl) internal view {
        ...
        require(
            IERC165Upgradeable(newImpl).supportsInterface(type(IUpgradable).interfaceId) &&
            IERC165Upgradeable(newImpl).supportsInterface(type(IERC1967Upgradeable).interfaceId),
    ...
    function executeUpgrade() external virtual onlyOwner whenNotLocked nonReentrant {
        ...
        previousImplementation = ERC1967UpgradeUpgradeable._getImplementation();
        upgradeInProgress = true;
        upgradeFailureTimestamp = block.timestamp + ROLLBACK_WINDOW;
        
        // Gunakan pola call setelah upgrade untuk hindari reentrancy
        ERC1967UpgradeUpgradeable._upgradeTo(pendingImplementation);
    ...
    function rollbackUpgrade() external onlyOwner nonReentrant {
        ...
        require(
            ERC1967UpgradeUpgradeable._getImplementation() == previousImplementation,
            "Rollback failed"
        );
...
TypeError: Wrong argument count for modifier invocation: 2 arguments given but expected 0.
   --> CoreV55.sol:209:9:
    |
209 |         EIP712Upgradeable("CoreV55", "5.5") 
    |         ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
TypeError: Invalid type for argument in function call. Invalid implicit conversion from bool to address requested.
   --> CoreV55.sol:303:30:
    |
303 |         if (_owners.contains(_verifySignature(userOpHash, userOp.signature))) {
    |                              ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^


TypeError: Type bool is not implicitly convertible to expected type address.
   --> CoreV55.sol:308:9:
    |
308 |         address signer = _verifySignature(userOpHash, userOp.signature);
    |         ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^


TypeError: Member "staticcall" not found or not visible after argument-dependent lookup in bytes calldata.
   --> CoreV55.sol:348:45:
    |
348 |  ... bool success, bytes memory data) = _remoteAddress.staticcall(abi.encodeWithSignature("supportsI ...
    |                                         ^^^^^^^^^^^^^^^^^^^^^^^^^
