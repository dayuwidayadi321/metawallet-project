 ...
 * [NEW] RoolBack System Upgrade Mechanism
 */
...
abstract contract CoreV55 is Initializable, EIP712, ReentrancyGuard {
    ...
    function sendCrossChain(
        ...
        // Estimate gas fee - get only the native fee (first return value)
        ...
        if (_gasPaymentToken == address(0)) {
            ...
            // Gunakan low-level call untuk transferFrom
    ...
    // Storage optimization with isolated plugin storage
    bytes32 private constant PLUGIN_STORAGE_SLOT = 
        keccak256("CoreV55.plugin.storage");
    ...
    struct PluginSystem {
        ...
        mapping(address => bool) pluginInitialized;
    }
    
    // Constants for security
    uint256 public constant MAX_PLUGIN_INIT_GAS = 500_000;
    uint256 public constant MAX_PLUGIN_EXEC_GAS = 300_000;
    bytes1 private constant SELFDESTRUCT_OPCODE = 0xff;
    ...
    function installPlugin(
        ...
        require(
            !_pluginSystem().pluginInitialized[plugin],
        ...
        if (initData.length > 0) {
            ...
            _pluginSystem().pluginInitialized[plugin] = true;
        }
    ...
    function upgradePlugin(
        ...
        // Make migration function external with gas limit
        (bool success, bytes memory result) = address(this).call{gas: MAX_PLUGIN_INIT_GAS}(
            abi.encodeWithSelector(
    ...
    /* ========== INTERNAL FUNCTIONS ========== */
    function _pluginSystem() internal pure returns (PluginSystem storage ps) {
        bytes32 slot = PLUGIN_STORAGE_SLOT;
        assembly {
            ps.slot := slot
        }
    }
    ...
    function _validatePlugin(address plugin) internal view virtual {
        ...
        // Check for dangerous bytecode
        bytes memory code = plugin.code;
        for (uint i = 0; i < code.length; i++) {
            require(code[i] != SELFDESTRUCT_OPCODE, "Plugin contains selfdestruct");
        }
    ...
    function _initializePlugin(
        ...
        (bool success, bytes memory result) = plugin.delegatecall{gas: MAX_PLUGIN_INIT_GAS}(initData);
        if (!success) {
            if (result.length > 0) {
                assembly {
    ...
    function _executePlugin(address plugin, bytes calldata data) internal {
        // Pre-check with staticcall and gas limit
        (bool preCheckSuccess, ) = plugin.staticcall{gas: MAX_PLUGIN_EXEC_GAS}(data);
        require(preCheckSuccess, "Plugin pre-check failed");
    
        // Execute with gas limit and isolation
        bool success;
        bytes memory result;
        bool wasLocked = env.isLocked;
        env.isLocked = true;
        
        (success, result) = plugin.delegatecall{gas: MAX_PLUGIN_EXEC_GAS}(data);
        
        env.isLocked = wasLocked;
        emit PluginExecuted(plugin, msg.sig, msg.sender, msg.value, success);
    ...
    function _migratePluginState(
        ...
        if (migrationData.length > 0) {
            (bool success, bytes memory result) = newPlugin.delegatecall{gas: MAX_PLUGIN_INIT_GAS}(migrationData);
            if (!success) {
                if (result.length > 0) {
                    assembly {
                        revert(add(result, 32), mload(result))
    ...
    error DangerousOpcodeDetected();
    error PluginGasLimitExceeded();
    ...
    function initiateRecovery(
        ...
        // 1. Input Validation
        require(block.timestamp <= deadline, "CoreV55: Signature expired");
        require(_guardianConfig.guardians.contains(msg.sender), "CoreV55: Caller not guardian");
        require(newOwners.length > 0, "CoreV55: No new owners provided");
        require(signatures.length >= _guardianConfig.threshold, "CoreV55: Insufficient signatures");
        
        // Validate new owners array (prevent duplicates)
        for (uint256 i = 0; i < newOwners.length; i++) {
            require(newOwners[i] != address(0), "CoreV55: Invalid owner address");
            for (uint256 j = i + 1; j < newOwners.length; j++) {
                require(newOwners[i] != newOwners[j], "CoreV55: Duplicate owner");
            }
        }
        ...
        require(
            ...
            "CoreV55: Recovery cooldown active"
        );
    
        // 3. Generate recovery hash with chain-specific binding
        bytes32 ownersHash = keccak256(abi.encode(newOwners));
        ...
        // 4. Nonce tracking with additional protection
        bytes32 recoveryId = keccak256(abi.encodePacked(recoveryHash, block.chainid, msg.sender));
        require(!usedSignatures[recoveryId], "CoreV55: Nonce already used");
        usedSignatures[recoveryId] = true;
    
        // 5. Optimized signature verification
        uint256 validSignatures;
        ...
        for (uint256 i = 0; i < signatures.length; i++) {
            ...
            // Check if signer is valid guardian and not duplicate
            if (_guardianConfig.guardians.contains(signer)) {
        ...
        require(validSignatures >= _guardianConfig.threshold, "CoreV55: Insufficient guardian approvals");
    
        // 6. Execute recovery with checks-effects-interactions pattern
    ...
    function _replaceOwners(address[] calldata newOwners) internal virtual {
        ...
        _owners.clear(); // More efficient if using OpenZeppelin's EnumerableSet
        
        // Add new owners
        for (uint i = 0; i < newOwners.length; i++) {
            require(newOwners[i] != address(0), "CoreV55: Invalid owner");
            require(_owners.add(newOwners[i]), "CoreV55: Owner already exists");
        }
    }
    ...
    function executeUpgrade() external virtual onlyOwner whenNotLocked {
        ...
        // Set flag upgrade sedang berlangsung
        upgradeInProgress = true;
        
        // Simpan implementasi saat ini sebagai backup
        previousImplementation = StorageSlot.getAddressSlot(_IMPLEMENTATION_SLOT).value;
        ...
        // Set timestamp untuk rollback window
        upgradeFailureTimestamp = block.timestamp;
        
        emit UpgradeInitiated(impl);
        emit UpgradeExecuted(impl);
    }
    
    // Fungsi untuk memeriksa kesehatan upgrade baru (lebih robust)
    function checkUpgradeHealth() public returns (bool) {
        ...
        if (!success || keccak256(data) != keccak256(bytes("5.5"))) {
            isHealthy = false;
        }
    
        // 2. Cek fungsi penting (simulasi validateUserOp)
        if (isHealthy) {
            try this.simulateValidateUserOp() returns (bool result) {
        ...
        // 3. Cek fungsi fallback
        if (isHealthy) {
            (bool success3,) = address(this).staticcall(
                abi.encodeWithSignature("nonExistingFunction()")
            );
            if (success3) isHealthy = false;
        }
    
        // 4. Cek fungsi penting lainnya (sesuai kebutuhan)
        if (isHealthy) {
            try this.simulateImportantFunction() returns (bool result) {
                if (!result) isHealthy = false;
            } catch {
                isHealthy = false;
            }
    ...
    function simulateValidateUserOp() external view returns (bool) {
        // Implementasi simulasi validateUserOp
        return true;
    }
    
    function simulateImportantFunction() external view returns (bool) {
        // Implementasi simulasi fungsi penting lainnya
        return true;
    }
    
    // Fungsi rollback yang lebih aman
    function rollbackUpgrade() external onlyOwner {
        ...
        // Verifikasi bahwa upgrade saat ini bermasalah
        bool isHealthy = checkUpgradeHealth();
        require(!isHealthy, "Current implementation is healthy");
        ...
        upgradeFailureTimestamp = 0;
        upgradeInProgress = false;
        
        emit UpgradeRolledBack(previousImplementation);
    }
    
    // Fungsi untuk mengkonfirmasi upgrade berhasil
    function confirmUpgrade() external onlyOwner {
        require(upgradeInProgress, "No upgrade in progress");
        
        bool isHealthy = checkUpgradeHealth();
        require(isHealthy, "Cannot confirm unhealthy upgrade");
        ...
        emit UpgradeConfirmed(StorageSlot.getAddressSlot(_IMPLEMENTATION_SLOT).value);
    }
    ...
    fallback() external payable virtual nonReentrant {
        ...
        if (selector == 0x9e5faafc || selector == 0x71e4150e) { // Contoh: block selfdestruct & delegatecall
