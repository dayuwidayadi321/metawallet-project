...
abstract contract CoreV55 is Initializable, UUPSUpgradeable, EIP712, ReentrancyGuard {
    ...
    function _validateImplementation(address newImpl) internal view {
        ...
        // Check Version dengan proteksi tambahan
        (bool success, bytes memory data) = newImpl.staticcall(
            abi.encodeWithSignature("VERSION()")
        );
        require(success, "Version check failed");
        require(data.length == 32, "Invalid version data");
        require(keccak256(data) == keccak256(bytes("5.5")), "Incompatible version");
        
        // Tambahkan pengecekan interface
        try IERC165(newImpl).supportsInterface(type(IERC165).interfaceId) returns (bool supported) {
            require(supported, "Doesn't support ERC165");
        } catch {
    ...
    function executeUpgrade() external virtual onlyOwner whenNotLocked {
        ...
        (bool success, ) = address(this).call(
            abi.encodeWithSignature("healthCheck()")
        );
        require(success, "Upgrade health check failed");
    ...
    function checkUpgradeHealth() public returns (bool) {
        // Hanya owner atau kontrak itu sendiri yang bisa memanggil
