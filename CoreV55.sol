...
abstract contract CoreV55 is Initializable, UUPSUpgradeable, EIP712, ReentrancyGuard {
    ...
    function checkUpgradeHealth() public returns (bool) {
        ...
        // 1. Version check dengan format ketat
        try this.version{gas: 50_000}() returns (string memory version) {
            if (bytes(version).length != 3 || keccak256(bytes(version)) != keccak256(bytes("5.5"))) {
                isHealthy = false;
            }
        } catch {
            isHealthy = false;
