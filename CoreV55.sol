...
abstract contract CoreV55 is Initializable, UUPSUpgradeable, EIP712, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(address => bool)) public usedChainNonces;
    ...
    function validateUserOp(
        ...
        if (_owners.contains(_verifySignature(userOpHash, userOp.signature))) {
            return 0;
        }
    ...
    function setTrustedRemote(uint16 _chainId, bytes calldata _remoteAddress) external onlyOwner {
        ...
        (bool success, bytes memory data) = _remoteAddress.staticcall(abi.encodeWithSignature("supportsInterface(bytes4)", type(ILayerZeroReceiver).interfaceId));
    ...
    function sendCrossChain(
        ...
        require(gasLimit <= 5_000_000, "Gas limit to high");
    ...
    function upgradePlugin(
        ...
        (bool success, bytes memory result) = address(this).call{gas: 1_000_000}(
            abi.encodeWithSelector(
    ...
    function _validatePlugin(address plugin, bytes4[] calldata selectors) internal view {
        ...
        // Cek ukuran kode yang wajar (antara 0.5KB - 24KB)
        uint256 size;
        assembly {
            size := extcodesize(plugin)
        }
        require(size > 512 && size < 24576, "Invalid plugin size");
        
        // Cek tidak mengandung delegatecall berbahaya
        bytes memory code = plugin.code;
        require(!_containsBytes(code, bytes4(0xb61d27f6)), "Contains dangerous delegatecall");
    }
    
    // Helper function untuk mencari bytecode tertentu
    function _containsBytes(bytes memory data, bytes4 search) internal pure returns (bool) {
        for (uint i = 0; i < data.length - 3; i++) {
            if (bytes4(data[i]) == search) {
                return true;
            }
        }
        return false;
    ...
    function _initializePlugin(
        ...
        require(gasBefore - gasleft() < 1_000_000, "Initialization used too much gas");
    ...
    function initiateRecovery(
        ...
        require(!usedRecoveryNonces[msg.sender][recoveryNonce], "CoreV55: Nonce already used");
        require(_guardianConfig.guardians.length() > 0, "CoreV55: No guardians configured");
        
        // Validasi cooldown period
        require(
            block.timestamp >= _guardianConfig.lastRecoveryTimestamp + _guardianConfig.cooldown,
            "CoreV55: Recovery cooldown active"
        );
    
        // Hitung hash dari newOwners untuk keamanan
        bytes32 ownersHash = keccak256(abi.encodePacked(newOwners));
        usedRecoveryNonces[msg.sender][recoveryNonce] = true;
        
        // Membuat recovery hash dengan EIP-712
    ...
    bytes32 recoveryId = keccak256(abi.encodePacked(recoveryHash, block.chainid, address(this), block.chainid));
        require(!usedSignatures[recoveryId], "CoreV55: Signature already used");
        ...
        for (uint256 i = 0; i < signatures.length; i++) {
            ...
            // Cek apakah signer adalah guardian yang valid dan bukan duplikat
            if (_guardianConfig.guardians.contains(signer)) {
                ...
                if (!alreadySeen) {
                    ...
                    // Keluar lebih awal jika threshold tercapai
        ...
        require(
            validSignatures >= _guardianConfig.threshold,
            "CoreV55: Insufficient guardian approvals"
        );
    ...
    function _validateImplementation(address newImpl) internal view {
        ...
        string memory versionString = abi.decode(data, (string));
        require(bytes(versionString).length == 3 && keccak256(bytes(versionString)) == keccak256(bytes("5.5")), "Incompatible version");
        
        // ERC165 check untuk interface yang diperlukan
        require(
            IERC165(newImpl).supportsInterface(type(IUpgradable).interfaceId) &&
            IERC165(newImpl).supportsInterface(type(IERC1967).interfaceId),
            "Missing required interfaces"
        );
    
        // Tambahkan pengecekan storage layout (opsional)
        if (newImpl.supportsInterface(type(IStorageCheck).interfaceId)) {
            require(IStorageCheck(newImpl).isStorageCompatible(address(this)), "Storage layout mismatch");
        }
    }
    ...
    function executeUpgrade() external virtual onlyOwner whenNotLocked nonReentrant {
        ...
        previousImplementation = ERC1967Upgrade._getImplementation();
        upgradeInProgress = true;
        upgradeFailureTimestamp = block.timestamp + ROLLBACK_WINDOW;
        
        // Gunakan pola call setelah upgrade untuk hindari reentrancy
        ERC1967Upgrade._upgradeTo(pendingImplementation);
    ...
    function checkUpgradeHealth() public returns (bool) {
        ...
        // 1. Version check with strict format
        try this.version{gas: 50_000}() returns (string memory versionString) {
            if (bytes(versionString).length != 3 || keccak256(bytes(versionString)) != keccak256(bytes("5.5"))) {
                isHealthy = false;
            }
        } catch {
            isHealthy = false;
        ...
        // 2. Cek fungsi kritis dengan batas gas
        if (isHealthy) {
            try this.simulateValidateUserOp{gas: 100_000}() returns (bool result) {
        ...
        // 3. Cek fallback dengan selector yang benar-benar unik
        if (isHealthy) {
            bytes memory payload = abi.encodeWithSignature("nonExistentABC123XYZ()");
            (bool success, bytes memory data) = address(this).staticcall{gas: 30_000}(payload);
            if (success || data.length > 0) {
                isHealthy = false;
            }
        }
    
        // 4. Cek storage layout dengan batas gas
        if (isHealthy && address(this).code.length > 0) {
    ...
    function rollbackUpgrade() external onlyOwner nonReentrant {
        ...
        // Validasi implementasi sebelumnya
        require(AddressUpgradeable.isContract(previousImplementation), "Previous impl no longer valid");
        
        bool isHealthy = checkUpgradeHealth();
        require(!isHealthy, "Current implementation is healthy");
    
        // Gunakan _upgradeTo yang aman
        ERC1967Upgrade._upgradeTo(previousImplementation);
        
        // Verifikasi rollback berhasil
        require(
            ERC1967Upgrade._getImplementation() == previousImplementation,
            "Rollback failed"
        );
    ...
    function confirmUpgrade() external onlyOwner {
        require(upgradeInProgress, "No upgrade in progress");
        require(block.timestamp <= upgradeTimestamp + CONFIRM_WINDOW, "Confirmation window expired");
        ...
        emit UpgradeConfirmed(ERC1967Upgrade._getImplementation());
    }
        ...
        whenNotLocked 
    {
        ...
        require(newImpl != ERC1967Upgrade._getImplementation(), "Same implementation");
        
        emit UpgradeAuthorized(newImpl); // Event khusus untuk tracking
    }
