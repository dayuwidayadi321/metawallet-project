...
abstract contract CoreV55 is Initializable, EIP712, ReentrancyGuard {
    ...
    function initiateRecovery(
        ...
        bytes32 ownersHash = keccak256(abi.encode(newOwners)); // Changed to abi.encode for better safety
        ...
        uint256 validSignatures;
        uint256 guardianCount = _guardianConfig.guardians.length();
        mapping(address => bool) storage guardians = _guardianConfig.guardians;
        mapping(address => bool) memory seenGuardians;
        
        for (uint256 i = 0; i < signatures.length; i++) {
            address signer = ECDSA.recover(recoveryHash, signatures[i]);
            
            // Check if signer is valid guardian and not duplicate
            if (guardians.contains(signer) && !seenGuardians[signer]) {
                seenGuardians[signer] = true;
                validSignatures++;
