...
import "./CoreV532.sol"; // <-- Perubahan nama file di sini

/**
 * @title SecureSmartWallet v5.3.2 - Enterprise Smart Wallet with ETH Withdrawal & Bundler Support
 * @author DFXC IndonesiaSecurity (Upgraded to Core v5.3.2)
 * @dev Key Features:
 * - Inherits Core v5.3.2 (Enhanced Security & EIP-4337)
 * - Native ETH Withdrawal & Bundler Integration
 * - Modular Design with Plugin Support
 * - Guardian Emergency Recovery
 */
contract SecureSmartWallet is Initializable, CoreV532, IERC1271Upgradeable, EIP712Upgradeable {
    /* ========== CONSTANTS ========== */
    string public constant NAME = "SecureSmartWallet";
    string public constant VERSION = "5.3.2"; // <-- Update versi
    ...
    constructor(IEntryPoint _entryPoint, address _gasOracle) 
        CoreV532(_entryPoint, _gasOracle) // <-- Perubahan nama kontrak induk
    {
    ...
    function initialize(
        ...
        // Initialize EIP-712
        __EIP712_init("SecureSmartWallet", "5.3.2"); // <-- Update versi
        
        // Initialize Core v5.3.2
        __CoreV532_init( // <-- Perubahan nama fungsi initializer
            _owners,
