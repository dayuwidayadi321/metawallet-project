 ...
 * @dev Enhanced security upgradeable smart wallet with:
 * - Multi-owner & guardian support
    ...
    IERC1271Upgradeable
{
    ...
    string public constant UPGRADE_NOTES = "Enhanced upgrade safety with codeHash verification";
    ...
    event UpgradeSafetyCheck(address indexed implementation, bool isSafe);
    ...
    // ========== Magic Number ============ //
    uint256 private constant USEROP_VALIDATIONDATA_OFFSET = 288;
    ...
    // ========== Enhanced Upgrade Functions ========== //
    
    /**
     * @notice Schedule a new implementation for upgrade (Enhanced)
     * @dev Added codeHash verification and logging
    ...
    /**
     * @notice Check upgrade safety off-chain
     * @dev New helper function for frontend validation
     * @param impl Address of the implementation to verify
     * @return bool True if implementation appears safe
     */
    function verifyUpgradeSafety(address impl) external view returns (bool) {
        if (impl == address(0)) return false;
        if (!impl.isContract()) return false;
        
        try IERC1822ProxiableUpgradeable(impl).proxiableUUID() returns (bytes32) {
            return true;
        } catch {
            return false;
        }
    ...
    /**
     * @notice Cancel pending upgrade (Enhanced)
     * @dev Added codeHash logging before cancellation
     */
    function cancelUpgrade() external onlyOwner {
        require(pendingImplementation != address(0), "No upgrade scheduled");
        
        // Log verification data before cancellation
        bytes32 codeHash;
        assembly {
            codeHash := extcodehash(pendingImplementation)
        }
        emit UpgradeVerified(pendingImplementation, codeHash);
    ...
    function _tryGetValidationDataLegacy(UserOperation calldata userOp) internal pure returns (uint48 validAfter, uint48 validUntil) {
        bytes memory userOpBytes = abi.encode(userOp);
        require(userOpBytes.length >= USEROP_VALIDATIONDATA_OFFSET + 32, "Invalid UserOp");
        
        assembly {
            // Skip 32 byte length prefix + offset 288
            let ptr := add(add(userOpBytes, 32), USEROP_VALIDATIONDATA_OFFSET)
            let validationData := mload(ptr)
            // Ekstrak 48-bit timestamps
