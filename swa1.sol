...
pragma solidity ^0.8.20; // Tetap menggunakan 0.8.20 seperti kode asli Anda
...
// HAPUS DEKLARASI INI KARENA SUDAH ADA DI IAccount/IEntryPoint
...
contract SmartWalletV2 is UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, IAccount {
    ...
    // Diperbaiki: Menggunakan PackedUserOperation sesuai dengan IAccount yang Anda impor
    function validateUserOp(
        PackedUserOperation calldata userOp, // <--- PERUBAHAN PENTING DI SINI
        bytes32 userOpHash,
        ...
        // Validate signatures
        require(userOp.signature.length >= 65 * requiredConfirmations, "SmartWallet: insufficient signatures");
        ...
        for (uint i = 0; i < requiredConfirmations; i++) {
            ...
            for (uint j = 0; j < 65; j++) {
                currentSig[j] = userOp.signature[i*65 + j];
            }
            ...
            for (uint k = 0; k < i; k++) {
                if (signedSigners[k] == signer) {
                    duplicate = true;
                    break;
                }
            }
            require(!duplicate, "SmartWallet: duplicate signature from same owner");
        ...
        return 0; // Return 0 untuk validasi sukses
    }

    // --- Core Functions ---
    // Diperbaiki: Menambahkan 'override' karena ini adalah bagian dari standar IAccount ERC-4337
    function execute(
        address dest,
        uint256 value,
        bytes calldata func
    ) external override onlyEntryPoint { // <--- TAMBAH override
    ...
    function removeOwner(address ownerToRemove) external onlyOwnerV2 {
        ...
        require(index != type(uint256).max, "SmartWallet: owner not found in array");
    ...
    // Nonce Function (Diperbaiki: menggunakan getNonce() dan override)
    // Diperbaiki: Menambahkan 'override' karena ini adalah bagian dari standar IAccount ERC-4337
    ...
    function submitTransaction(
        ...
        bytes32 txHash = keccak256(abi.encodePacked(to, value, data, block.chainid, address(this), getNonce()));
...
contract SmartWalletFactoryV2 {
    ...
    constructor(address _entryPoint) {
        entryPoint = _entryPoint;
        // Pastikan SmartWalletV2 di-deploy dengan entryPoint yang benar
