...
contract SecureSmartWallet is IERC1271, Initializable, UUPSUpgradeable, ReentrancyGuard {
    ...
    /**
     * @notice Get guardian threshold
     */
    function getGuardianThreshold() external view returns (uint256) {
        return guardianConfig.threshold;
    }
    ...
    function isGuardian(address addr) external view returns (bool) {
        return guardianConfig.isGuardian[addr];
    }
    
    /**
     * @notice Get guardian cooldown period
     */
    function getGuardianCooldown() external view returns (uint256) {
        return guardianConfig.cooldown;
    }
    
    /**
     * @notice Get guardian nonce
     */
    function getGuardianNonce() external view returns (uint256) {
        return guardianConfig.nonce;
    }    
    ...
    // ========== ERC1271 Implementation Optimized ========== //
    function _validateSignature(bytes32 hash, bytes memory signature) internal view returns (bool) {
        ...
        return recovered == owner || isOwner[recovered]; // Support both single owner & multi-owner
    }
