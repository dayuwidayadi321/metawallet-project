...
/**
 * @title SecureSmartWallet - EIP-4337 Smart Wallet (v4.45)
 ...
 * - Fixed guardian detection issues in deployment and login
 */
contract SecureSmartWallet is IERC1271, Initializable, UUPSUpgradeable, ReentrancyGuard {
    ...
    string public constant VERSION = "4.45";
    ...
    function initialize(
        ...
        // Initialize guardians - FIXED: Proper initialization of guardian mapping
    ...
    function validateUserOp(
        ...
        // FIXED: Enhanced signature validation for guardian detection
        if (!_validateSignature(userOpHash, userOp.signature)) {
            // Additional check for guardian signatures
    ...
    function _validateGuardianSignature(bytes32 hash, bytes memory signature) internal view returns (bool) {
        address recovered = hash.recover(signature);
        return guardianConfig.isGuardian[recovered];
    }
    ...
    // ========== Version 4.45 Specific Fixes ========== //
    function fixGuardianDetection() external onlyOwner {
        // This function ensures guardian mapping is properly initialized
        for (uint256 i = 0; i < guardianConfig.list.length; i++) {
            if (!guardianConfig.isGuardian[guardianConfig.list[i]]) {
                guardianConfig.isGuardian[guardianConfig.list[i]] = true;
            }
        }
...
contract SecureSmartWalletFactory {
    ...
    function deployWallet(
        ...
        // Explicitly call fixGuardianDetection on the new wallet
        SecureSmartWallet(wallet).fixGuardianDetection();
