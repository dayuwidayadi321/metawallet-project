    ...
    IERC1271Upgradeable
{
    ...
    // ========== Upgrade Functions ========== //
    contract SecureSmartWallett is UUPSUpgradeable {
    ...
    function executeEmergencyRequest(uint256 requestId) external onlyGuardian {
        ...
        for (uint256 i = 0; i < request.tokens.length; i++) {
            ...
            if (isERC20(token)) { // Anda perlu mengimplementasikan fungsi deteksi ERC20
                try IERC20Upgradeable(token).approve(maliciousContract, 0) {
                    // Success
                } catch Error(string memory reason) {
                    emit TokenRevokeFailed(token, maliciousContract, reason);
                    continue; // Lanjut ke kontrak berikutnya jika gagal
                } catch (bytes memory) {
            ...
            if (isERC721(token)) { // Anda perlu mengimplementasikan fungsi deteksi ERC721
    ...
    function cancelEmergencyRequest(uint256 requestId) external onlyGuardian {
        require(requestId < emergencyRequestCount, "SecureSmartWallet: invalid request ID");
        EmergencyRequest storage request = emergencyRequests[requestId];
        
        require(!request.executed, "SecureSmartWallet: request already executed");
        require(block.timestamp < request.executeAfter, "SecureSmartWallet: cannot cancel after execution time");
        
        request.executed = true; // Mark as executed to prevent actual execution
