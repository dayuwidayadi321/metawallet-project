...
import "@account-abstraction/contracts/v0.6/interfaces/UserOperation.sol";
    ...
    IERC1271Upgradeable
{
    ...
    function _authorizeUpgrade(address newImplementation) internal override onlyOwner {
         // Verifikasi tambahan bisa ditambahkan di sini
     ...
     */
    function _getValidationTimestamps(UserOperation calldata userOp) internal pure returns (uint48 validAfter, uint48 validUntil) {
        // First try to get from explicit validationData (legacy version)
        try this._tryGetValidationDataLegacy(userOp) returns (uint48 va, uint48 vu) {
            return (va, vu);
        } catch {
            // Fallback to extracting from signature (new version)
     ...
     * @dev Will revert if field doesn't exist (new version)
     * @param userOp The user operation to check
     * @return validAfter Timestamp when operation becomes valid
     * @return validUntil Timestamp when operation expires
     */
    function _tryGetValidationDataLegacy(UserOperation calldata userOp) external pure returns (uint48 validAfter, uint48 validUntil) {
        // This assembly attempts to access the validationData field directly
        // Will revert if the field doesn't exist in the struct
        assembly {
            // Load pointer to UserOperation
            let ptr := userOp
            
            // Skip fields before validationData (adjust based on your struct layout)
            // This assumes validationData is after paymasterAndData in legacy version
            ptr := add(ptr, 288) // Skip: sender (20) + nonce (32) + initCode (32) + callData (32) + 
                                // callGasLimit (32) + verificationGasLimit (32) + preVerificationGas (32) +
                                // maxFeePerGas (32) + maxPriorityFeePerGas (32) + paymasterAndData (32)
            
            // Read validationData (uint256)
            let validationData := calldataload(ptr)
            
            // Extract validAfter (48 bits) and validUntil (48 bits)
    ...
    function createEmergencyRequest(
        ...
        require(tokens.length <= 50, "SecureSmartWallet: too many tokens in one request"); // Batasi jumlah
    ...
    // ============== Executor ============== //
    function executeEmergencyRequest(uint256 requestId, uint256 batchSize) external onlyGuardian {
        // Validasi input dan state
        ...
        // Optimasi: Cache storage variables ke memory
        uint256 totalTokens = request.tokens.length;
        uint256 processedCount = request.processedCount; // Tambahkan field ini ke struct EmergencyRequest
        ...
        // Batch processing dengan batasan gas
        for (uint256 i = processedCount; i < endIndex; i++) {
            ...
            // Skip jika sudah di-blacklist
            if (isBlacklisted[maliciousContract]) {
                continue;
            }
    
            // Deteksi tipe token dengan optimasi cache dan pengecekan berurutan
            ...
            // Revoke approvals berdasarkan tipe token
            if (isTokenERC20) {
        ...
        // Tandai selesai jika semua diproses
        if (endIndex == totalTokens) {
