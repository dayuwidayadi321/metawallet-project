    ...
    function validateUserOp(
        ...
        if (bytes4(userOp.callData) == this.executeScheduledOperation.selector) {
            ...
            if (op.executed || block.timestamp < op.executeAfter) {
                return _packValidationData(true, 0, 0); // Operasi belum valid
            }
            return _packValidationData(false, 0, 0); // Operasi valid
        }
        ...
        if (!isValidOwner && !isValidGuardian && recovered.code.length > 0) {
            try IERC1271Upgradeable(recovered).isValidSignature(userOpHash, userOp.signature) 
                ...
                // Tetap false jika signature invalid
            }
        }
    
        // 6. Validasi otorisasi
        if (!isValidOwner && !isValidGuardian) {
            return _packValidationData(true, 0, 0); // Signature invalid
        }
    
        // 7. Validasi waktu (validAfter/validUntil)
        (uint48 validAfter, uint48 validUntil) = _parseValidationData(userOp.validationData);
        
        if (validAfter > 0 && block.timestamp < validAfter) {
            return _packValidationData(true, validAfter, validUntil); // Belum valid
        }
        
        if (validUntil > 0 && block.timestamp > validUntil) {
            return _packValidationData(true, validAfter, validUntil); // Expired
        }
    
        return _packValidationData(false, validAfter, validUntil); // Valid
    }
    
    // Helper functions
    function _parseValidationData(uint256 validationData) internal pure returns (uint48 validAfter, uint48 validUntil) {
        validAfter = uint48(validationData >> 160);
        validUntil = uint48(validationData >> (160 + 48));
    }
